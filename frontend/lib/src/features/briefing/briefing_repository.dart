...
class BriefingRepository extends _$BriefingRepository {
  double? _parseSentiment(dynamic s) {
    ...
    if (s is String) {
      ...
      if (lowerS.contains('very bullish') || lowerS.contains('strong buy'))
        return 0.95;
      if (lowerS.contains('bullish') ||
          lowerS.contains('buy') ||
          lowerS.contains('positive'))
        return 0.75;
      if (lowerS.contains('neutral') ||
          lowerS.contains('hold') ||
          lowerS.contains('mixed'))
        return 0.0;
      if (lowerS.contains('bearish') ||
          lowerS.contains('sell') ||
          lowerS.contains('negative'))
        return -0.75;
      if (lowerS.contains('very bearish') || lowerS.contains('strong sell'))
        return -0.95;
  ...
  String _formatCategory(String key) {
    ...
    if (key == 'market_analyst' || key == 'market_analysis')
      return 'Market Analysis';
    if (key == 'opportunity_scout' ||
        key == 'opportunities' ||
        key == 'high_signal_divergent')
  ...
  Map<String, dynamic> _normalizeItem(Map<String, dynamic> map) {
    ...
    if (result['price_action_projection'] != null &&
        result['potential_price_action'] == null) {
    ...
    if (result['analysis'] is Map<String, dynamic>) {
      ...
      final outlook =
          analysis['market_outlook']?.toString() ??
      ...
      final priceAction =
          analysis['potential_price_action']?.toString() ??
          analysis['price_action_projection']?.toString();

      if (result['takeaway'] == null && outlook != null)
        result['takeaway'] = outlook;
      if (result['catalysts'] == null && catalysts != null)
        result['catalysts'] = catalysts;
      if (result['risks'] == null && risks != null) result['risks'] = risks;
      if (result['potential_price_action'] == null && priceAction != null)
        result['potential_price_action'] = priceAction;
    }
    ...
    if (result['analysis'] != null && result['analysis'] is! String) {
      if (result['analysis'] is Map) {
        ...
        result['analysis'] =
            analysisMap['market_outlook']?.toString() ??
    ...
    for (final field in listStringFields) {
      if (result[field] != null) {
        if (result[field] is List) {
          result[field] = (result[field] as List)
              .map((e) => e.toString())
              .toList();
      ...
      if (response.statusCode == 200) {
        ...
        if (decodedBody is List) {
          if (decodedBody.isEmpty) {
            return const BriefingData(
              data: {},
              success: true,
              message: 'No briefing data found.',
            );
          }
          ...
          if (decodedBody['data'] is List &&
            ...
            firstBriefing =
                (decodedBody['data'] as List).first as Map<String, dynamic>;
          } else {
        ...
        if (rawData is String) {
          ...
          return const BriefingData(
            data: {},
            success: true,
            message: 'Briefing data is empty.',
          );
        } else {
        ...
        if (rawContent.trim().isEmpty) {
          return const BriefingData(
            data: {},
            success: true,
            message: 'Empty briefing content.',
          );
        }
        ...
        if (decodedContent is Map<String, dynamic>) {
                ...
                container.forEach((key, value) {
                  if (value is List) {
                    try {
                      ...
                      final List<BriefingItem> items = value
                          .map(
                            (i) => BriefingItem.fromJson(
                              _normalizeItem(i as Map<String, dynamic>),
                            ),
                          )
                          .toList();
                      ...
                      final firstTakeaway = items
                          .where(
                            (i) => i.takeaway != null && i.takeaway!.isNotEmpty,
                          )
                          .firstOrNull
                          ?.takeaway;
                      categoriesMap[categoryName] = CategoryData(
                        sentimentScore: count > 0
                            ? totalSentiment / count
                            : 0.0,
                        summary:
                            firstTakeaway ??
                            '${items.length} items in $categoryName.',
                        items: items,
                      );
                    } catch (e) {
                      debugPrint(
                        'BriefingRepository: Failed to parse news category $key: $e',
                      );
                    }
                  }
                });
          ...
          for (final containerKey in marketContainers) {
            if (decodedContent.containsKey(containerKey)) {
              ...
              if (itemsList.isNotEmpty) {
                try {
                  final List<BriefingItem> items = itemsList
                      .map(
                        (i) => BriefingItem.fromJson(
                          _normalizeItem(i as Map<String, dynamic>),
                        ),
                  ...
                  for (final item in items) {
                    final score = _parseSentiment(
                      item.sentimentScore ?? item.sentiment,
                    );
                    if (score != null) {
                      totalSentiment += score;
                      count++;
                  ...
                  final firstAnalysis = items
                      .where(
                        (i) => i.analysis != null && i.analysis!.isNotEmpty,
                      )
                      .firstOrNull
                      ?.analysis;
                  categoriesMap['Market Analysis'] = CategoryData(
                    ...
                    summary:
                        firstAnalysis ?? '${items.length} tickers analyzed.',
                    items: items,
                  );
                } catch (e) {
                  debugPrint(
                    'BriefingRepository: Failed to parse $containerKey: $e',
                  );
                }
              }
            }
            ...
            if (decodedContent.containsKey(containerKey)) {
              ...
              if (itemsList.isNotEmpty) {
                try {
                  ...
                  final firstExplanation = items
                      .where(
                        (i) =>
                            i.explanation != null && i.explanation!.isNotEmpty,
                      )
                      .firstOrNull
                      ?.explanation;
                  categoriesMap['Opportunities'] = CategoryData(
                    ...
                    summary:
                        firstExplanation ??
                        '${items.length} opportunities identified.',
                    items: items,
                  );
                } catch (e) {
                  debugPrint(
          ...
          decodedContent.forEach((key, value) {
            ...
            if (newsContainers.contains(key) ||
                ...
                opportunityContainers.contains(key)) {
              return;
            }
            ...
            if (value is Map<String, dynamic>) {
              try {
                ...
                debugPrint(
                  'BriefingRepository: Failed to parse generic category map $categoryName: $e',
                );
              }
              ...
              try {
                ...
                final firstItemText = items
                    .where((i) => i.takeaway != null && i.takeaway!.isNotEmpty)
                    .firstOrNull
                    ?.takeaway;
                categoriesMap[categoryName] = CategoryData(
                  sentimentScore: count > 0 ? totalSentiment / count : 0.0,
                ...
                debugPrint(
                  'BriefingRepository: Failed to parse generic list $categoryName: $e',
                );
              }
            }
          });
        ...
        return BriefingData(
          ...
          createdAt: firstBriefing['createdAt'] != null
              ? DateTime.parse(firstBriefing['createdAt'])
              : null,
        );
      } else {
